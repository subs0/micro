...
const updateDocker = ({ path, runtime, handler, docker }) => {
    ...
    const { dockerfile = 'Dockerfile', platform = 'linux/x86_64', config = [], } = !isPlainObject(docker) ? { dockerfile: 'Dockerfile', platform: 'linux/x86_64' } : docker;
...
const configurations = ({ source, apex, tags, builder = '${path.root}/package.py', build_dir = '${path.root}/builds', }) => {
    ...
    const configs = dirs.reduce((a, d) => {
        ...
        if (hasConfig) {
            ...
            const { name, runtime, handler, memory = 128 * 8, timeout = 60, tmp_storage = 512, architectures = ['x86_64'], sns, s3, cloudwatch, api, docker, tags: _tags, } = config;
