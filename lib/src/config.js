 ...
 */
const exportFinalizer = (obj, path, scoped) => {
    const warn = (path) => {
            ...
            return {
                ...
                [k]: v.map((x, i) => {
                    if (x == 'undefined' || x == 'null')
                        warn([...path, k, i]);
                    if (isString(x) && x.includes('$SCOPE')) {
                        // for depends_on: [...]
                        return x.replace(/\$SCOPE/g, scoped);
                    }
                    if (isPlainObject(x))
                        return exportFinalizer(x, [...path, k, i], scoped);
                    else
                        return x;
                }),
            };
    ...
    return Object.entries(obj).reduce((a, c) => {
        ...
        if (pivotPoints.includes(key)) {
            ...
            return refs
                ...
                : {
                    ...
                    [key]: {
                        ...
                        [type]: {
                            ...(a[key] && a[key][type]),
                            [scoped]: exportFinalizer(target, [key, raw_type], scoped),
                        },
                    },
                };
        }
            ...
            return {
                ...
                [key]: val.map((x, i) => {
                    if (isPlainObject(x)) {
                        return flattenPreservingPaths(x, provider, [...path, key, i], a, refs);
                    }
                    else {
                        return x;
                    }
