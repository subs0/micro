import { PIVOT_POINTS, GLOBALS, ROOT_MEMBERS } from './constants';
 ...
 *    (refs === false)
 */
export const fold = ({ target, provider, path = [], refs = false, out = {}, globals = [] }) => {
    if (target === null) {
        return; // SKIP IT
    }
    if (!path.length) {
 ...
 */
export const modulate = (obj, provider = 'aws', globals = []) => {
