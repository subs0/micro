...
memory_size = 128, timeout = 60, env_vars = {}, tags = {}, depends_on = [], tmp_storage, bucket_env, role_arn, sns, }, my) => {
    ...
    if (!has_msg_attrs)
        console.log(name, 'lambda not publishing');
    const is_subscribed = someIncludes(sns, 'filter_policy') ? true : false;
    if (!is_subscribed)
        console.log(name, 'lambda not subscribed');
    ...
    */
    return {
        ...
        ...(sns && is_subscribed
            ? sns.reduce((acc, { filter_policy, ref, topic_key }) => ({
                ...acc,
