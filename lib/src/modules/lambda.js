...
const bucket_policy_statement = ({ bucket_name, role_arn = '' }) => ({
...
const multi_stmt_policy_doc = ({ bucket_name = '', topic_arn = '', cloudwatch_arn = '', role_arn = '', }) => ({
    data: {
        iam_policy_document: {
            statement: [
                ...(bucket_name
                    ? [bucket_policy_statement({ bucket_name, role_arn })]
                    : []),
                ...(topic_arn
                    ? [
                        {
 ...
 */
const lambda_fn = ({ name, role_arn, file_path, env_vars = {}, handler, runtime, package_type = 'Zip', tmp_storage, architectures = ['x86_64'], timeout = 60, memory_size = 128, depends_on = [], tags = {}, log_group_name = '', // TODO
 }) => ({
    resource: {
        lambda_function: {
            package_type,
 ...
 * ```
 */
export const lambda = ({ name = 'microservice', runtime, handler, file_path = '${path.root}/functions/template/zipped/handler.py.zip', architectures = ['x86_64'], memory_size = 128, timeout = 60, env_vars = {}, tags = {}, depends_on = [], tmp_storage, sns, }, my) => {
