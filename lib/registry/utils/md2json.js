...
import { nn_h2, nn_h3, nn_h4, nn_h5, tick_group, head_rx, cleanBody, required, } from './regex';
 ...
 *
 */
export const recursivePropCapture = (md, arg = 'Argument Reference', attr = 'Attribute Reference', step = 0, seps = [nn_h2, nn_h3, nn_h4, nn_h5]) => {
    if (step === seps.length)
        return md;
    ...
    const results = parts.reduce((a, c) => {
        ...
        if (!heading)
            return a;
        const chunk = cleanBody(c.replace(head_rx, '')); //
 ...
 */
export const separateAttrsArgsAndDedupProps = (payload, arg = 'Argument Reference', attr = 'Attribute Reference') => {
...
const test_json_w_md = readFileSync(`registry/docs/terraform-provider-aws/${v}/${debug_id}.json`, 'utf8');
