...
exports.compileTypes = exports.generateInterfacesForProvider = exports.triageQTSampleProps = exports.cleanKey = void 0;
const fs_1 = require("fs");
const quicktype_core_1 = require("quicktype-core");
const parse_root_provider_1 = require("./parse-root-provider");
const decorate_types_1 = require("./decorate-types");
const constants_1 = require("./constants");
const checks_1 = require("@thi.ng/checks");
const cleanKey = (str) => str.trim().replace(/\s|-/g, '_').replace('!', '');
 ...
 */
const triageQTSampleProps = (input, max_depth = 1, depth = 0, output = {}) => {
    if (depth === max_depth) {
    ...
    return Object.entries(input).reduce((a, c, i, d) => {
        const [key, val] = c;
        if ((0, constants_1.isRequired)(key)) {
            if ((0, checks_1.isPlainObject)(val)) {
                // required, add key to accumulator and recurse over object
                return {
                    ...a,
                    [(0, exports.cleanKey)(key)]: (0, exports.triageQTSampleProps)(val, max_depth, depth + 1),
                };
            }
            else {
                // required, include string value in current accumulator
                return { ...a, [(0, exports.cleanKey)(key)]: val };
            }
        }
        else {
            /**
            ...
            return {
                ...a,
                ...(0, exports.triageQTSampleProps)({ [`${key}!`]: val }, max_depth, depth + 1),
            };
        }
    }, output);
};
exports.triageQTSampleProps = triageQTSampleProps;
/**
...
const recursivelyGenerateSamplesForQT = (merged = {}, provider = 'terraform-provider-aws', version = '43475', path = 'registry/json') => {
    ...
    let max_depth = 1;
    let sample = (0, exports.triageQTSampleProps)(merged, max_depth);
    let samples = [sample];
    // skip even numbers (presents required children with parent)
    while (JSON.stringify(samples[(max_depth + 1) / 2 - 1]).length <
        JSON.stringify((0, exports.triageQTSampleProps)(merged, max_depth + 2)).length) {
        max_depth = max_depth + 2;
        const s = (0, exports.triageQTSampleProps)(merged, max_depth);
        samples.push(s);
    }
    if (!(0, fs_1.existsSync)(jsonPath)) {
        (0, fs_1.mkdirSync)(jsonPath);
    }
    const JSON_samples = samples.map((x) => JSON.stringify(x, null, 4));
    const out = JSON_samples.map((x, i) => {
        (0, fs_1.writeFileSync)(`${jsonPath}/sample${i}.json`, x);
        return x;
    });
    return out;
};
...
const getQtTypesFromProviderSamples = async (samples, provider = 'terraform-provider-aws', targetLanguage = 'typescript', indent = 4) => {
    ...
    const { lines, ...rest } = await (0, quicktype_core_1.quicktype)({
...
const generateInterfacesForProvider = async (merged, provider = 'terraform-provider-aws', version = '43475', refresh = false, jsonPath = 'registry/json', typePath = 'registry/types') => {
    const typesPath = `${typePath}/${provider}/${version}/types.ts`;
    if (!refresh && (0, fs_1.existsSync)(typesPath)) {
        return (0, fs_1.readFileSync)(typesPath, 'utf8').split('\n');
    }
    ...
    (0, fs_1.writeFileSync)(typesPath, interfaces.join('\n'));
    return interfaces;
};
exports.generateInterfacesForProvider = generateInterfacesForProvider;
/**
    ...
    const cut = (entries, output = {}) => entries.length
        ? () => {
            ...
            if ((0, checks_1.isPlainObject)(val)) {
                return cut(rest, {
                    ...output,
                    [(0, exports.cleanKey)(key)]: barber(val),
                });
            }
            else {
                return cut(rest, { ...output, [(0, exports.cleanKey)(key)]: val });
            }
        }
        : output;
...
const compileTypes = async (provider = 'terraform-provider-aws', version = '5.20.0', refresh = false, reload = false, typePath = 'registry/types') => {
    const v = constants_1.VERSIONS[provider][version] || '43475';
    const skips = { '43475': ['3226064'], '43126': ['3199143'] }[v] || [];
    console.log(`\nGenerating JSON from Docs for: ${provider} ...\n`);
    const jsonDoc = await (0, parse_root_provider_1.saveJsonDocForRootSpec)(provider, version, refresh, reload, skips).then((x) => {
    ...
    const interfaces = await (0, exports.generateInterfacesForProvider)(merged, provider, version, refresh);
    const cleancut = barber(merged); //?
    const augmentedLines = (0, decorate_types_1.typeLinesAugmenter)(interfaces, cleancut).join('\n');
    ...
    (0, fs_1.writeFileSync)(augmentedPath, augmentedLines);
    console.log('🚀 DONE 🚀');
};
