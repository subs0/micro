                ...
                if commands:
                    ...
                    if pip_requirements and runtime.startswith('python'):
                        if isinstance(pip_requirements, bool) and path:
                            pip_requirements_step(
                                path, prefix, required=True, tmp_dir=claim.get('pip_tmp_dir'))
                        else:
                    ...
                    if npm_requirements and runtime.startswith('nodejs'):
                        if isinstance(npm_requirements, bool) and path:
                            npm_requirements_step(
                                path, prefix, required=True, tmp_dir=claim.get('npm_tmp_dir'))
                        else:
...
def install_poetry_dependencies(query, path):
    ...
    log.info("Installing python dependencies with poetry & pip: %s",
             poetry_lock_file)
    with tempdir() as temp_dir:
        ...
        if os.path.isfile(poetry_lock_file):
            ...
            poetry_lock_target_file = copy_file_to_target(
                poetry_lock_file, temp_dir)
        else:
            poetry_lock_target_file = None

        if os.path.isfile(poetry_toml_file):
            log.info("Using poetry configuration file: %s", poetry_lock_file)
            poetry_toml_target_file = copy_file_to_target(
                poetry_toml_file, temp_dir)
        else:
            ...
            if docker:
                ...
                if poetry_cache_dir:
                    if isinstance(poetry_cache_dir, str):
                        ...
                        poetry_cache_dir = os.path.abspath(
                            os.path.join(
                                working_dir, artifacts_dir, "cache/poetry")
                        )
...
def prepare_command(args):
    ...
    recreate_missing_package = yesno_bool(
        args.recreate_missing_package if args.recreate_missing_package is not None else query.recreate_missing_package)
    docker = query.docker
